...
class Crawler:
    def __init__(self, storage_state_path: Path):
        ...
        if storage_state_path:
            self.page = self.browser.new_page(storage_state=storage_state_path.as_posix())
        else:
    ...
    def crawl(self):
        ...
        for index, node_name_index in enumerate(node_names):
            ...
            ancestor_node_key = (
                None if not ancestor_exception else str(anchor_id) if is_ancestor_of_anchor else str(button_id)
            )
        ...
        for element in elements_in_view_port:
            ...
            element.get("origin_x")
            element.get("origin_y")
...
def parse_args():
    ...
    parser.add_argument("-s", "--storage-state-path", type=Path, help="Session storage file to use.")
    parser.add_argument(
        "-q",
        "--quiet",
        action="store_true",
...
if __name__ == "__main__":
    ...
    if quiet:
        print(
            "Running in quiet mode hides HTML and other content; \n"
            + "Please exercise caution when running suggested commands."
        )

    _crawler = Crawler(args.storage_state_path)
